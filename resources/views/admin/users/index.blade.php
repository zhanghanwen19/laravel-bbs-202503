...
@section('description', __('Users Overview'))
    ...
    <div class="tw-bg-white tw-shadow-xl sm:tw-rounded-lg tw-p-6">
        <div class="tw-mb-6 tw-flex tw-flex-col md:tw-flex-row tw-justify-between tw-items-center tw-gap-4">
            ...
            <div class="tw-flex-1 tw-w-full md:tw-w-auto">
                <form action="{{ route('admin.users.index') }}" method="GET" class="tw-flex tw-items-center tw-gap-2">
            ...
            <div class="tw-w-full md:tw-w-auto">
                <form action="{{ route('admin.users.index') }}" method="GET">
                    ...
                    <select name="sort_by"
                        ...
                        <option value="last_updated" {{ request('sort_by', 'last_updated') == 'last_updated' ? 'selected' : '' }}>{{ __('Last Updated') }}</option>
                        <option value="latest_registered" {{ request('sort_by') == 'latest_registered' ? 'selected' : '' }}>{{ __('Latest Registered') }}</option>
                    </select>
                </form>
            </div>
        </div>
            ...
            <div class="tw-overflow-x-auto tw-shadow-md tw-rounded-lg tw-border tw-border-gray-200">
                <table class="tw-min-w-full tw-divide-y tw-divide-gray-200">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="tw-px-6 tw-py-3 tw-text-right tw-text-xs tw-font-medium tw-text-gray-500 tw-uppercase tw-tracking-wider">
                            {{ __('Actions') }}
                        </th>
                    </tr>
                    </thead>
                    ...
                    @foreach ($users as $user)
                        <tr class="hover:tw-bg-gray-50 tw-transition-colors tw-duration-150">
                            ...
                            <td class="tw-px-6 tw-py-4 tw-whitespace-nowrap tw-text-right tw-text-sm tw-font-medium">
                                <a href="{{ route('admin.users.show', $user) }}" class="tw-text-indigo-600 hover:tw-text-indigo-900 tw-mr-3 tw-inline-flex tw-items-center tw-gap-1">
                                    <svg class="tw-h-4 tw-w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    {{ __('Details') }}
                                </a>
                                <a href="{{ route('admin.users.edit', $user) }}" class="tw-text-green-600 hover:tw-text-green-900 tw-mr-3 tw-inline-flex tw-items-center tw-gap-1">
                                    <svg class="tw-h-4 tw-w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    {{ __('Edit') }}
                                </a>
                                <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="tw-inline" onsubmit="return confirm('{{ __('Are you sure you want to delete this user?') }}');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="tw-text-red-600 hover:tw-text-red-900 tw-inline-flex tw-items-center tw-gap-1">
                                        <svg class="tw-h-4 tw-w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        {{ __('Delete') }}
                                    </button>
                                </form>
                            </td>

...
@section('description', __('Create New Category'))
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}">
            ...
            <div class="tw-mb-6 tw-bg-gray-50 tw-rounded-lg tw-p-6 tw-shadow-inner">
                ...
                <div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-6">
                    {{-- 名称 --}}
                    <div>
                        <label for="name" class="tw-block tw-text-sm tw-font-medium tw-text-gray-700 tw-mb-1">{{ __('Category name') }}</label>
                    ...
                    <div class="md:tw-col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="tw-block tw-w-full tw-rounded-md tw-border-gray-300 tw-shadow-sm focus:tw-ring-indigo-500 focus:tw-border-indigo-500 tw-text-base tw-px-4 tw-py-2">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="tw-mt-1 tw-text-sm tw-text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
